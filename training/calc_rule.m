function [ rule_info ] = calc_rule( train,C,alphabet,combos_c,Ethresh,h0,prior )
...
switch numf
        ...
        for f2=1:length(alpha2)
            ...
            if e >= Ethresh % calc dx
                ...
                if abs(d) >= h0  %generate rule
                    ...
                    rule_info=[rule_info;dum WOE d  dum o 2 featvalues];
                end;
            end;
        end;
    case 3
            ...
            for f3=1:length(alpha3)
                ...
                if e >= Ethresh % calc dx
                    ...
                    if abs(d) >= h0  %generate rule
                        ...
                        rule_info=[rule_info;dum WOE d  dum o 3 featvalues];
                    end;
                end;
            end;
        end;
                ...
                for f4=1:length(alpha4)
                    ...
                    if e >= Ethresh % calc dx
                        ...
                        if abs(d) >= h0  %generate rule
                            ...
                            rule_info=[rule_info;dum WOE d  dum o 4 featvalues];
                        end;
                    end;
                end;
            end;
                    ...
                    for f5=1:length(alpha5)
                        ...
                        if e >= Ethresh % calc dx
                            ...
                            if abs(d) >= h0  %generate rule
                                ...
                                rule_info=[rule_info;dum WOE d  dum o 5 featvalues];
                            end;
                        end;
                    end;
                end;
    ...
    case 6
        for f2=1:length(alpha2)
            for f3=1:length(alpha3)
                for f4=1:length(alpha4)
                    for f5=1:length(alpha5)
                        for f6=1:length(alpha6)
                            of1=length(find((A(:,1)==C)));
                            of2=length(find((A(:,2)==alpha2(f2))));
                            of3=length(find((A(:,3)==alpha3(f3))));
                            of4=length(find((A(:,4)==alpha4(f4))));
                            of5=length(find((A(:,5)==alpha5(f5))));
                            of6=length(find((A(:,6)==alpha6(f6))));
                            e = N * (of1/N) * (of2/N)* (of3/N)* (of4/N)* (of5/N) * (of6/N);
                            if e >= Ethresh % calc dx
                                o=length(find((A(:,1)==C)&(A(:,2)==alpha2(f2))&(A(:,3)==alpha3(f3))&(A(:,4)==alpha4(f4))&...
                                    (A(:,5)==alpha5(f5))&(A(:,6)==alpha6(f6))));
                                d=calc_dx( o,e,N );
                                if abs(d) >= h0  %generate rule
                                    owc=length(find((A(:,2)==alpha2(f2))&(A(:,3)==alpha3(f3))&(A(:,4)==alpha4(f4))&...
                                        (A(:,5)==alpha5(f5))&(A(:,6)==alpha6(f6))));
                                    ...
                                    for f=1:length(combos_c);
                                        switch f
                                            ...
                                            case 5
                                                featvalues(combos_c(f))=alpha5(f5);
                                            case 6
                                                featvalues(combos_c(f))=alpha6(f6);
                                        end;
                                    end;
                                    rule_info=[rule_info;dum WOE d  dum o 6 featvalues];
                                end;
                            end;
                        end;
                    end;
    ...
    otherwise
        error('6 is the highest number of features inlcuding class calc_rule can handle')
end;
